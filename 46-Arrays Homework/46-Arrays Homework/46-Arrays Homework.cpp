...
float CalculateGradesAverage(float grades[3])  //Arrays are ALWAYS passed by reference automatically (Adding & will result in syntax error). 
{
...
int main()
{
	...
	float GradesAverage = CalculateGradesAverage(grades); //The passed argument 'grades' also contains the array's memory address. Hence, the params do NOT need the & sign. 
