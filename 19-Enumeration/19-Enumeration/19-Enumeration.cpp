...
/* Added note December 24th, 2025:
   
   You have an integer value in the code regardless of how it came to be 
   there whether by:

   - Initialization/ reading user input.
   - Generated by the code itself.
   - Received by the code.
   - Fetched from database
   - Fetched from a file in the fraemwork
   - etc.

   so regardless of why it exists in the code, WHERE
   this integer value is not going to be used in an arithmetic operation,
   but is going to be used further in the code representing a word, concept,
   a state or an idea.

   The further use is in, for example, a conditional statement.
   where instead of 
   
   - explicitly mentioning this raw number

   we, instead, 
   - Create an enum user-defined data type.
   - The naturally, declare a variable.
   - Then naturally, initialize it (Directly or via reading user input).

   Ex: assume that the number 55 in my code represents Egypt, then:

   if (55 == ...)    // implying that you neither created an enum, nor used it in your program.

   // a week or an hour later, looking at the if statement will not be conducive
   to remembering what 55 is and this would cause many errors in the code especially
   that your program may consist easily of more than a thousand lines of code.
