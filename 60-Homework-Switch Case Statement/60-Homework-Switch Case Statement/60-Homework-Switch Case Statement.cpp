...
float PerformOperation(const enOperationType &CastedC, const stOperation &operation)
{
	switch (CastedC)
	{
...
int main()
{
	...
	// operation & c are now initialized but:
	// for c, we need to convert its data type from char to enOperationType
	// because this SINGLE value of c will be compared to the enumeration constants (enumerators) acting as labels
...
int main()
{
	...
	// storing the value of c in variable WeekDays so that
	...
	string SelectedDay = SelectWeekDay(WeekDays); // argument is the user entered numerical value that has its data type casted to enWeekDays (from int).
