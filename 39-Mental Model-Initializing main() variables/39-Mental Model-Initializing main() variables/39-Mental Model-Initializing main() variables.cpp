...
/*
        ...
        The key goal is to ensure that variables declared in main() are initialized in a way that keeps their values inside main()’s scope —
        not inside any other set of curly braces that do not belong to main().
        Proper initialization guarantees that these values remain usable for the rest of the program.
 --------------------------------------------------------------------------------------------------------------------------------------------------
...
// create a procedure (which is something we know), & pass parameters by ref. (instead of creating new variables whose scope is the procedure
